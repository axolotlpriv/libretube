...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    private fun showRecommended(streamItems: List<StreamItem>?) {
        if (streamItems.isNullOrEmpty()) {
            binding.recommendedRV.isVisible = false
            binding.recommendedTV.isVisible = false
            return
        }

        makeVisible(binding.recommendedRV, binding.recommendedTV)
        recommendedAdapter.submitList(streamItems.take(20))
    }
