...
import com.github.libretube.ui.dialogs.TvSignInDialog
...
class InstanceSettings : BasePreferenceFragment() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        val tvAuthStore = TvAuthStore(requireContext())
        val tvSignIn = findPreference<Preference>("tv_sign_in")
        val tvSignOut = findPreference<Preference>("tv_sign_out")

        fun refreshTvSignInUI() {
            val loggedIn = tvAuthStore.isLoggedIn()
            tvSignIn?.isVisible = !loggedIn
            tvSignOut?.isVisible = loggedIn
        }
        refreshTvSignInUI()

        tvSignIn?.setOnPreferenceClickListener {
            val dialog = TvSignInDialog()
            dialog.show(childFragmentManager, TvSignInDialog::class.java.name)
            // token is saved asynchronously inside the dialog; refresh the pref UI
            // once the user backs out / it dismisses on success
