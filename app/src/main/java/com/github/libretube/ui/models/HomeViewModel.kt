...
class HomeViewModel : ViewModel() {
    ...
    // "Recommended for you": personalized feed pulled from the real YouTube
    // account, only populated when signed in via TV OAuth (see TvAuthStore).
    ...
    private val sections
        get() = listOf(trending, feed, bookmarks, playlists, continueWatching, recommended)
        ...
        loadHomeJob = viewModelScope.launch {
            val result = async {
                awaitAll(
                    ...
                    async { if (visibleItems.contains(RECOMMENDED)) loadRecommended(context) }
                )
